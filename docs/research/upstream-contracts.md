...
- 请求可能跨过 reset。先以完整响应读取结束时间匹配返回窗口；若服务端返回
  reset 前生成的旧窗口，则回退到请求开始时间。只有请求区间两端都不属于
  返回的严格窗口时才拒绝，最终选中的匹配时刻作为 `captured_at`。
...
不能阻断数据库启动，也不能覆盖最新合格观测。若旧 source health 的最后
