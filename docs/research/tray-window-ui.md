...
默认态只显示周摘要、额度窗口日期轨道与重置预测摘要；某天的使用、上限、可用和状态在
...
- 再次点击托盘图标切换显示/隐藏。
- 默认概览尽量不滚动；额度窗口日期轨道、雷达摘要与最后成功时间同时可见，展开明细时允许
  中间内容区短距离滚动。
- 设置内容内部滚动，顶部标题/返回和底部“完成”固定。
- 过期或告警条加入后，优先保留全部额度窗口日期完整可见；必要时只有中间内容区产生短距离滚动，底部导航不被覆盖。
...
4. **本周七日策略**
   - 标题显示窗口起止日期；
   - 按精确窗口展示全部相交自然日期；午夜重置通常七格，非午夜重置通常八格；
